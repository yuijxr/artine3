document.addEventListener('DOMContentLoaded', () => {
  ...
  function notify(msg, type = 'info'){
    try {
        if (typeof showNotification === 'function') showNotification(msg, type); else alert(msg);
  ...
  async function fetchAllAddresses(){
    try{
      ...
      return addressesCache;
        ...
        console.error(err); addressesCache = []; return []
    }
  }
  ...
  function renderPreview(list){
    ...
    if(!Array.isArray(list) || list.length === 0){
        addrContainer.innerHTML = '<div style="color:#666">No saved addresses</div>'; return;
    }
    ...
    for(const a of list){
        if(preview.length >= 2) break; if(def && a.address_id==def.address_id) continue; preview.push(a);
    }
    ...
    preview.forEach(a => {
      ...
      main.innerHTML = `<strong>${escapeHtml(a.full_name)}</strong> <span style="color:#666">(${escapeHtml(a.phone)})</span><div style="margin-top:6px;color:#333">${escapeHtml(a.street)}</div><div style="margin-top:4px;color:#666;font-size:13px">${escapeHtml(a.city)}, ${escapeHtml(a.province)} ${escapeHtml(a.postal_code||'')} ${escapeHtml(country)}</div>`;
      const actions = document.createElement('div'); actions.className = 'addr-actions';
      if(Number(a.is_default) === 1 || a.is_default === '1') actions.innerHTML = '<span class="default-badge">Default</span>';
      // clicking preview card sets it as default (quick select)
      card.appendChild(main); card.appendChild(actions);
      main.addEventListener('click', async ()=>{
        try{
          const r = await fetch('api/addresses.php', { method:'PUT', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ address_id: a.address_id, set_default: 1 }) });
          const j = await r.json(); if(j && j.success){
              notify('Default updated','success'); const list2 = await fetchAllAddresses(); renderPreview(list2);
          } else { notify('Could not set default','error'); }
        }catch(err){
            notify('Failed','error');
        }
      });
  ...
  function renderManagerList(list){
    ...
    if(!Array.isArray(list) || list.length === 0){
        managerList.innerHTML = '<div style="color:#666">No saved addresses</div>'; return;
    }
    list.forEach(a => {
      const el = document.createElement('div'); el.className = 'address-card';
      const main = document.createElement('div'); main.className = 'addr-main';
      const country = (!a.country || a.country === '0') ? 'Philippines' : a.country;
      main.innerHTML = `<strong>${escapeHtml(a.full_name)}</strong> ${Number(a.is_default)===1 ? '<span class="default-badge">Default</span>' : '' }<div style="margin-top:6px;color:#333">${escapeHtml(a.street)}</div><div style="margin-top:4px;color:#666;font-size:13px">${escapeHtml(a.city)}, ${escapeHtml(a.province)} ${escapeHtml(a.postal_code||'')} ${escapeHtml(country)}</div>`;
      const actions = document.createElement('div'); actions.className = 'addr-actions';
      // Edit icon button
      const editBtn = document.createElement('button'); editBtn.className = 'icon-btn'; editBtn.title = 'Edit'; editBtn.innerHTML = '<i class="fa fa-pen"></i>';
      editBtn.addEventListener('click', (ev)=>{ ev.stopPropagation(); openFormForAddress(a); });
      // Delete icon button
      const delBtn = document.createElement('button'); delBtn.className = 'icon-btn danger'; delBtn.title = 'Delete'; delBtn.innerHTML = '<i class="fa fa-trash"></i>';
      delBtn.addEventListener('click', async (ev)=>{ ev.stopPropagation(); if(!confirm('Delete this address?')) return; try{ const r = await fetch('api/addresses.php', { method:'DELETE', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ address_id: a.address_id }) }); const j = await r.json(); if(j && j.success){
          notify('Deleted','success'); const list2 = await fetchAllAddresses(); // adjust pending selection if needed
                                                          ...
                                                          renderManagerList(list2); renderPreview(list2);
      } else notify('Delete failed: ' + (j && j.message ? j.message : ''),'error'); }catch(err){ notify('Delete failed','error'); } });

      const wrapActions = document.createElement('div'); wrapActions.style.display='flex'; wrapActions.style.gap='8px';
  wrapActions.appendChild(editBtn); wrapActions.appendChild(delBtn);
    ...
    if(pendingSelectedAddressId){
      Array.from(managerList.children).forEach((ch, idx) => {
        ...
        if(addr && String(addr.address_id) === String(pendingSelectedAddressId)){
            ch.classList.add('selected'); ch.style.borderColor = '#1976d2'; ch.style.boxShadow = '0 0 0 3px rgba(25,118,210,0.06)';
        }
      });
    }
  }
  ...
  async function saveAddress(ev){
    ...
    try{
      if(id){ payload.address_id = id; const r = await fetch('api/addresses.php', { method:'PUT', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) }); const j = await r.json(); if(j && j.success){
          notify('Address updated','success'); const list2 = await fetchAllAddresses(); renderManagerList(list2); renderPreview(list2); managerListWrap.style.display = ''; form.style.display = 'none';
      } else notify('Save failed','error'); }
      else { const r = await fetch('api/addresses.php', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) }); const j = await r.json(); if(j && j.success){
          notify('Address saved','success'); const list2 = await fetchAllAddresses(); renderManagerList(list2); renderPreview(list2); managerListWrap.style.display = ''; form.style.display = 'none';
      } else notify('Save failed','error'); }
    }catch(err){
        console.error(err); notify('Save failed','error');
    }
  }
  ...
  btnBack && btnBack.addEventListener('click', (e)=>{ e.preventDefault(); form.style.display = 'none'; managerListWrap.style.display = ''; });
  ...
  modalCancelBtn && modalCancelBtn.addEventListener('click', (e)=>{ e.preventDefault(); // discard selection and close
    e.stopPropagation(); pendingSelectedAddressId = null; closeModal(); });
  modalSaveBtn && modalSaveBtn.addEventListener('click', async (e)=>{
    ...
    try{
      const r = await fetch('api/addresses.php', { method:'PUT', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ address_id: pendingSelectedAddressId, set_default: 1 }) });
      ...
      if(j && j.success){
          ...
          if(checkoutCard){
            ...
            if(addr){
              ...
              checkoutCard.innerHTML = `\
                <div class="address-name">${escapeHtml(addr.full_name)} <span class="address-phone">(${escapeHtml(addr.phone)})</span></div>\
                <div class="address-details">${escapeHtml(addr.street)}, ${escapeHtml(addr.city)}, ${escapeHtml(addr.province)}</div>\
                <div class="address-details">${escapeHtml(addr.postal_code)}, ${escapeHtml(addr.country)}</div>\
        ...
        console.error(err); notify('Could not set default','error');
    }
  });
