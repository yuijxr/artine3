...
async function loadCheckout(){
    ...
    if (typeof window !== 'undefined' && window.INIT_CHECKOUT_DATA) {
        ...
        try{
            delete window.INIT_CHECKOUT_DATA;
        }catch(e){}
    } else {
    ...
    function needsModal(name){
        return /gcash|credit|card/i.test(name || '');
    }
    if (pmContainer) {
            ...
            const card = document.createElement('div');
                ...
                card.addEventListener('click', (e)=>{
                    ...
                    if (!needsModal(m.name)){
                        ...
                        showPaymentModal(m, (extraData)=>{
                            if (extraData !== null) selectCard();
                        });
                    }
                });
...
function showPaymentModal(method, onConfirm){
    ...
    if (!modal) {
        if (typeof onConfirm === 'function') onConfirm(null); return;
    }
    ...
    if (/gcash/i.test(name)){
        ...
        html = `<div>No extra information required for ${
            method.name
        }.</div>`;
    }
    body.innerHTML = html;
    ...
    function hide(){
        modal.setAttribute('aria-hidden','true'); removeListeners();
    }
    function removeListeners(){
        cancel.removeEventListener('click', onCancel); confirm.removeEventListener('click', onConfirmClick); closeBtn.removeEventListener('click', onCancel);
    }
    function onCancel(){
        hide(); if (typeof onConfirm === 'function') onConfirm(null);
    }
    function onConfirmClick(){
        // basic validation
        let extra = null;
        const f = document.getElementById('pm-field');
        if (f){ if (!f.value.trim()){
            alert('Please fill required field'); return;
        } extra = f.value.trim(); }
    ...
    if (placeBtn) {
        placeBtn.addEventListener('click', async ()=>{
            try {
                ...
                if (!addrNode) {
                    if (typeof showNotification === 'function') showNotification('Please add a delivery address first.', 'error'); else alert('Please add a delivery address first.'); return;
                }
                ...
                if (!payment_method_id) {
                    if (typeof showNotification === 'function') showNotification('Please select a payment method.', 'error'); else alert('Please select a payment method.'); return;
                }
                ...
                if (j.success){ if (typeof showNotification === 'function') showNotification('Order placed!', 'success'); setTimeout(()=>{
                    window.location.href = 'account.php#orders';
                },1200); }
                else {
                    if (typeof showNotification === 'function') showNotification(j.message || 'Order failed', 'error'); else alert(j.message || 'Order failed');
                }
            } catch (err) {
                if (typeof showNotification === 'function') showNotification('Failed to place order: ' + err.message, 'error'); else alert('Failed to place order: ' + err.message);
            }
        });
    }
});
