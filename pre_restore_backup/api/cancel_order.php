...
if ($method !== 'POST') {
    echo json_encode(['success'=>false,'message'=>'Unsupported method']); exit;
}

$data = json_decode(file_get_contents('php://input'), true) ?: [];
$order_id = intval($data['order_id'] ?? 0);
if ($order_id <= 0) {
    echo json_encode(['success'=>false,'message'=>'order_id required']); exit;
}

// Verify ownership
$stmt = $conn->prepare('SELECT status, user_id FROM orders WHERE order_id = ?');
...
// perform update
$up = $conn->prepare('UPDATE orders SET status = ? WHERE order_id = ? AND user_id = ?');
$new = 'cancelled';
$up->bind_param('sii', $new, $order_id, $user_id);
if (!$up->execute()) {
    echo json_encode(['success'=>false,'message'=>'Failed to cancel: '.$up->error]); exit;
}
