...
if ($method === 'POST') {
    ...
    $stmt = $conn->prepare('INSERT INTO addresses (user_id,full_name,phone,street,city,province,postal_code,country,is_default,created_at) VALUES (?,?,?,?,?,?,?,?,0,NOW())');
...
if ($method === 'PUT') {
    ...
    $postal_code = $data['postal_code'] ?? '';
    $country = $data['country'] ?? '';
    $stmt = $conn->prepare('UPDATE addresses SET full_name=?,phone=?,street=?,city=?,province=?,postal_code=?,country=? WHERE address_id=? AND user_id=?');
