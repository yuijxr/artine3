...
if ($method === 'GET') {
    ...
    if ($order_id>0) {
        ...
        $stmt = $conn->prepare('SELECT oi.*, p.image_url, p.thumbnail_images, c.name AS category_name FROM order_items oi LEFT JOIN products p ON oi.product_id = p.product_id LEFT JOIN categories c ON p.category_id = c.category_id WHERE oi.order_id = ?');
        ...
        foreach ($rows as $r) {
            ...
            if (!$thumb && !empty($r['image_url'])) $thumb = $r['image_url'];
            ...
            $r['thumbnail'] = $thumb;
            if ($thumb) $r['thumbnail_path'] = 'assets/img/' . $folder . $thumb; else $r['thumbnail_path'] = 'assets/img/thumbnails/noimg.png';
            $items[] = $r;
        }
    ...
    foreach ($orders as &$ord) {
        $thumb = $ord['thumbnail'] ?? null;
                                                ...
                                                elseif (strpos($cat, 'cap') !== false) $folder = 'caps/';
                                                elseif (strpos($cat, 'perfume') !== false) $folder = 'perfumes/';
                                                if ($thumb) $ord['thumbnail_path'] = 'assets/img/' . $folder . $thumb; else $ord['thumbnail_path'] = 'assets/img/thumbnails/noimg.png';
    }
