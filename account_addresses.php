...
<body>
    ...
    async function loadAddrs(){
        ...
        container.querySelectorAll('.del').forEach(b=> b.addEventListener('click', async ()=>{
            ...
            try{
                ...
                if (j && j.success) { loadAddrs(); }
                else { if (typeof showNotification === 'function') showNotification('Delete failed: ' + (j && j.message ? j.message : 'unknown'), 'error'); else alert('Delete failed: ' + (j && j.message ? j.message : 'unknown')); }
            }catch(err){ if (typeof showNotification === 'function') showNotification('Delete failed: ' + err.message, 'error'); else alert('Delete failed: ' + err.message); }
        }));
        ...
        container.querySelectorAll('input[name="default_addr"]').forEach(r=> r.addEventListener('change', async ()=>{
            ...
            try{
                ...
                if (j && j.success) { loadAddrs(); }
                else { if (typeof showNotification === 'function') showNotification('Could not set default', 'error'); else alert('Could not set default'); }
            }catch(err){ if (typeof showNotification === 'function') showNotification('Could not set default: ' + err.message, 'error'); else alert('Could not set default: ' + err.message); }
        }));
    }
    ...
    document.addEventListener('DOMContentLoaded', ()=>{
        ...
        document.getElementById('save-addr').addEventListener('click', async (e)=>{ 
            ...
            if (id) { payload.address_id = id; const r = await fetch('api/addresses.php',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}); const j = await r.json(); if (j.success) { document.getElementById('addr-form-wrap').style.display='none'; loadAddrs(); } else { if (typeof showNotification === 'function') showNotification('Save failed', 'error'); else alert('Save failed'); } }
            else { const r = await fetch('api/addresses.php',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}); const j = await r.json(); if (j.success) { document.getElementById('addr-form-wrap').style.display='none'; loadAddrs(); } else { if (typeof showNotification === 'function') showNotification('Save failed', 'error'); else alert('Save failed'); } }
        }); });
    </script>
</body>
</html>
